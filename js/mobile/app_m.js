...
document.addEventListener("DOMContentLoaded", ()=>{
    ...
    function loadSwiperImgs(bannerImgs){
       for(let i=0;i<bannerImgs.length;i++){
          var swiperSlide = mkDOM("div",[{class:"swiper-slide"}]),imgNo=i+1,swiperImg =mkDOM("img",[{class:"img-banner"},{src:'./images/'+bannerImgs[i]},{alt:'Banner '+imgNo}]);
    ...
    const makeBoxMenuItems = (_obj)=>{
        let _li=mkDOM("li",[{id:_obj.href},{class:"box-item-info"}]),_div=mkDOM("div",[{class:"img-icon"}])
            ,_img=mkDOM("img",[{src:"./images/"+_obj.icon}]),_p=mkDOM("p",[{innerHTML:_obj.title}])
    ...
    const makeItemInfo = (_obj) =>{
        ...
        _img.setAttribute("src","./images/"+_obj.icon);_p.innerHTML=_obj.title;
